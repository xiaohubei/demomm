...
cb.rest.ContextBuilder = {
    construct: function (callback) {
        ...
        var cUserName = cb.data.CookieParser.getCookie('cUserName');
        var token = cb.data.CookieParser.getCookie('token');
        var inviteCode = new cb.util.queryString().get('inviteCode');
        if (inviteCode) {
            cb.data.CookieParser.setCookie('inviteCode', inviteCode);
        } else {
            inviteCode = cb.data.CookieParser.getCookie('inviteCode');
        }
        cb.rest.AppContext = $.extend(true, {}, appContext, { cUserName: cUserName, token: token, inviteCode: inviteCode });
