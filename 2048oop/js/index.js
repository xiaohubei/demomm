cb.views.register("NumBlockGameController", function (controllerName){
	...
	view.prototype.init = function (){
		...
		this.score = 0;
		this.mobileData = { startX : 0, startY : 0, endX : 0,endY : 0};
		this.envDetection();
	};
	view.prototype.envDetection = function (cb){
		var documentWidth=window.screen.availWidth; 
		if (documentWidth < 500) {
	        this.setMobileStyle();
	        this.mobileEventRegister();
	    } else {
	        this.gridContainerWidth = 500;
	        this.cellBlockWidth = 100;
	        this.cellGap = 20
	    };
		this.newGame();
	};
	view.prototype.setMobileStyle = function (){
	    $("#grid-container").css("width", this.gridContainerWidth - 2 * this.cellGap);
	    $("#grid-container").css("height", this.gridContainerWidth - 2 * this.cellGap);
	    $("#grid-container").css("padding", this.cellGap);
	    $("#grid-container").css("border-radius", 0.02 * this.gridContainerWidth);
	    $(".grid-cell").css("width", this.cellBlockWidth);
	    $(".grid-cell").css("height", this.cellBlockWidth);	
	};
	view.prototype.mobileEventRegister = function (){
		var self = this;
		document.addEventListener("touchstart",function(ev) {
		    self.mobileData.startx = ev.touches[0].pageX;
		    self.mobileData.starty = ev.touches[0].pageY
		});
		document.addEventListener("touchmove",function(ev) {
		    ev.preventDefault()
		});
		document.addEventListener("touchend",function(ev) {
		    self.mobileData.endx = ev.changedTouches[0].pageX;
		    self.mobileData.endy = ev.changedTouches[0].pageY;
		    var deltax = self.mobileData.endx - self.mobileData.startx;
		    var deltay = self.mobileData.endy - self.mobileData.starty;
		    if (Math.abs(deltax) < 0.2 * self.documentWidth && Math.abs(deltay) < 0.2 * self.documentWidth) {
		        return;
		    };
			...
			for( var j = 0; j<4; j++ ){
				...
				if( this.numBlockData[i][j] == 0 ){
					...
					numCell.css({
						'width':'0px',
						'height':'0px',
						'left':this.getPostLeft(i,j) + this.cellBlockWidth/2,
						'top':this.getPostTop(i,j) + this.cellBlockWidth/2
					});
				}else{
					...
					numCell.css({
						...
						'font-size':this.cellGap
					});
					//把要展示到画板的数字转换为数字
	                numCell.text( this.getNumberText( this.numBlockData[i][j] ) );
	                //numCell.text(  this.numBlockData[i][j]  );
				}
				this.collisionRecordData[i][j]=false;// 每次事件过后，他都要是没碰撞的状态
			}
		}
	    $(".number-cell").css("line-height", this.cellBlockWidth + "px");
	};
